...
import com.we.exchange.service.ExchangeRepayPlanService;

@Service("userProfitSchedule")
public class UserProfitSchedule implements IScheduleTaskDealSingle<Object> {

    private static final Logger logger = LoggerFactory.getLogger(UserProfitSchedule.class);

    @Autowired
    private ExchangeRepayPlanService exchangeRepayPlanService;

    @Override
    public List<Object> selectTasks(String arg0, String arg1, int arg2, List<TaskItemDefine> arg3, int arg4) throws Exception {
        List<Object> list = null;
        try {
            logger.info("--------user profit schedule()-----------");
            /**
             * 调用远程任务处理，有两点需要说明：
            ...
            this.exchangeRepayPlanService.userProfitSchedule();
            ...
            logger.error("UserProfitSchedule.selectTasks occured exception.", e);
        } finally {
            /**
             * 此处为固定写法
             */
