...
@Service("productFailedRefundSchedule")
public class ProductFailedRefundSchedule implements IScheduleTaskDealSingle<Object> {

    private static final Logger logger = LoggerFactory.getLogger(ProductFailedRefundSchedule.class);
    ...
    public List<Object> selectTasks(String taskParameter, String ownSign, int taskItemNum, List<TaskItemDefine> taskItemList, int eachFetchDataNum)
        ...
        try {
            logger.info("--------product failed to refund schedule()-----------");
            /**
             * 调用远程任务处理，有两点需要说明：
            ...
            this.exchangeProductService.productFailedToRefundSchedule();
