...
import com.we.exchange.service.ExchangeUserProductService;

@Service("userProductSchedule")
public class UserProductSchedule implements IScheduleTaskDealSingle<Object> {

    private static final Logger logger = LoggerFactory.getLogger(UserProductSchedule.class);

    @Autowired
    private ExchangeUserProductService exchangeUserProductService;
    ...
    public List<Object> selectTasks(String taskParameter, String ownSign, int taskItemNum, List<TaskItemDefine> taskItemList, int eachFetchDataNum)
        ...
        try {
            logger.info("--------user product schedule()-----------");
            /**
             * 调用远程任务处理，有两点需要说明：
            ...
            this.exchangeUserProductService.calculateInvestAmount();
            ...
            logger.error("UserProductSchedule.selectTasks occured exception.", e);
        } finally {
            /**
             * 此处为固定写法
             */
