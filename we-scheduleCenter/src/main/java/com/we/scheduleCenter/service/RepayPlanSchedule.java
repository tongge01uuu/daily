...
@Service("repayPlanSchedule")
public class RepayPlanSchedule implements IScheduleTaskDealSingle<Object> {

    private static final Logger logger = LoggerFactory.getLogger(RepayPlanSchedule.class);
    ...
    public List<Object> selectTasks(String taskParameter, String ownSign, int taskItemNum, List<TaskItemDefine> taskItemList, int eachFetchDataNum)
        ...
        try {
            logger.info("--------repay plan schedule()-----------");
            /**
             * 调用远程任务处理，有两点需要说明：
            ...
            this.exchangeRepayPlanService.repayPlanSchedule();
            ...
            logger.error("RepayPlanSchedule.selectTasks occured exception.", e);
        } finally {
            /**
             * 此处为固定写法
             */
