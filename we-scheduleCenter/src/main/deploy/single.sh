...
MAIN_CLASS=${MAIN_CLASS:-"com.we.scheduleCenter.ScheduleCenterServiceRunner"}
...
start(){
		if exist; then
	    	...
	    	cd $APP_PATH
				nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $CLASS_PATH $MAIN_CLASS $APP_PATH > $STDOUT_FILE 2>&1 &
                echo java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $CLASS_PATH $MAIN_CLASS $APP_PATH > $STDOUT_FILE 2>&1 &
